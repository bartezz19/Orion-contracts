...
contract Treasury is Ownable {
    ...
    address public sORION;
    uint public sORIONQueue; // Delays change to sORION address
    ...
    if ( _ORIONUSDC != address(0) ) {
        isLiquidityToken[ _ORIONUSDC ] = true;
        liquidityTokens.push( _ORIONUSDC );
    }
    ...
    /**
        @notice allow approved address to deposit an asset for ORION
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ORION needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ORION ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ORION for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        IORIONERC20( ORION ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sORION ).balanceOf( msg.sender ); // Can only borrow against sORION held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ORION
        @param _amount uint
     */
    function repayDebtWithORION( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IORIONERC20( ORION ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ORION, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ORION ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ORION ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ORION valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ORION decimals
            value_ = _amount.mul( 10 ** IERC20( ORION ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SORION ) { // 9
            sORIONQueue = block.number.add( blocksNeededForQueue );
        } else return false;
