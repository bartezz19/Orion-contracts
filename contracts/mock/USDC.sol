...
contract USDC is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    mapping (address => uint)                      public USDCMintedToday;
    ...
    function adjustDailyUSDCLimit(uint _limit) external auth {
    ...
    function mint(address usr, uint wad) external {
      ...
      if(wards[msg.sender] == 0) {
        require(add(wad, USDCMintedToday[msg.sender]) <= dailyUSDCLimit || sub(block.number, lastMintRestart[msg.sender]) >= 6500 && wad <= dailyUSDCLimit, "Over daily USDC Limit");
