...
task("deploy_aggbond", "Deploy AGG Bond")
    ...
    const bondDepository = await BondDepository.deploy(
        ...
        treasury.address,
        DAO_ADDRESS,
        bondConfig.PRICE_FEED_ADDRESS, { gasLimit: 5e6 } // Gas estimation may fail
    );
    ...
    // queue and toggle bond as REWARD MANAGER
    await treasury.queue('8', bondDepository.address, { gasLimit: 5e6 });
    console.log("treasury queue 8 success")
    await treasury.toggle('8', bondDepository.address, ZERO_ADDRESS, { gasLimit: 5e6 });
